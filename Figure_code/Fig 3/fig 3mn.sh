...
for (celltype in celltypes) {
  ...
  file_path <- paste0(gene, "/", intron, "@", SNP, "/", celltype, "@", intron, "@", SNP, ".txt")
  ...
  col_phe <- as.character(unlist(read.table(paste0("phename/", celltype, "_phename.txt"), sep = " ", check.names = FALSE)))
...
output_dir <- paste0("", gene, "/", intron, "@", SNP)
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
output_file <- paste0(output_dir, "/", gene, "@", intron, "@", SNP, ".jpg")
