...
celltypes <- c("BNav", "BMem", "CD4EM", "CD4Naive", "CD4Treg", "CD8Naive",
...
color_mapping <- c(
  ...
  "MAIT" = "#FFEE58", "MonocM" = "#6A1B9A", "NoClaM" = "#E1BEE7", "NKBright" = "#82581F",
  "NKDim" = "#D7CCC8"
)
...
for (celltype in celltypes) {
  ...
  file_path <- paste0("", gene, "/phe/cellty/", gene, "/", intron, "@", SNP, "/", celltype, "@", intron, "@", SNP, ".txt")
  ...
  geno_phe <- intersect(as.character(colnames(genotype)), as.character(colnames(phenotype)))[-1]
  merged_data <- rbind(
...
output_dir <- paste0(gene, "/figure/", gene, "/", intron, "@", SNP)
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)
output_file <- paste0(output_dir, "/", gene, "@", intron, "@", SNP, ".pdf")
