...
# Addresses droplet co-encapsulation doublets mentioned in methodology
# Key modifications:
...
paramSweep <- function(seu, PCs=1:10, sct = FALSE, num.cores=1) {
  ...
  # Remove pK values resulting in <1 neighbor (too small for meaningful analysis)
  min.cells <- round(nrow(seu@meta.data)/(1-0.05) - nrow(seu@meta.data))
  pK.test <- round(pK*min.cells)
  pK <- pK[which(pK.test >= 1)]

  # Store original Seurat commands for consistent preprocessing
...
# Used when ground-truth doublet classifications are unavailable

find.pK <- function(sweep.stats) {
  '%ni%' <- Negate('%in%')
  
  # Check if AUC column exists (indicates ground-truth data available)
...
removeDoublet = function(seurat_obj) {
  ...
  # Accounts for 10x Chromium technology-specific doublet rates
  DoubletRate = as.numeric(opt$doublerate)
  
  # Adjust for homotypic doublets (same cell type)
  # Homotypic doublets are harder to detect, so expected count is adjusted
