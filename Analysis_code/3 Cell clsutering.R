...
# Parameters align with methodology's batch effect correction and cell type annotation steps
library(optparse)
option_list = list(
...
library(Seurat)
library(patchwork)
library(harmony)        # For batch effect correction as mentioned in methodology
library(tidyverse)
library(clustree)
library(cowplot)
...
library(stringr)
library(ggplot2)
options(future.globals.maxSize = 1000 * 1024^3)  # Increase memory limit for large datasetsData Loading and Cleanupr# Load aggregated Seurat object from all pools
# This object contains cells that passed DoubletFinder and QC filters:
#   - >= 500 UMIs
#   - >= 250 detected genes
#   - < 25% mitochondrial content
#   - Met OneK1K QC standards
...
# Optional: Handle missing cell type annotations
# sce@meta.data$ct_cov[sce@meta.data$ct_cov == ""] <- "unknown"
# Data Normalization and Feature Selectionr# Log-normalization as specified in methodology:
...
# Scales all genes (not just variable features) for comprehensive analysis
...
sce <- RunPCA(object = sce, 
              ...
              npcs = 50)Harmony Batch Effect Correctionr# Apply Harmony for batch effect correction as specified in methodology:
...
sce <- sce %>% 
  RunHarmony(group.by.vars = "POOL")  # POOL = batch/pool identifier from opt$batchssNeighbor Finding and UMAP Dimensionality Reductionr# Find k-nearest neighbors in Harmony-corrected space
...
sce <- sce %>% 
  ...
  RunUMAP(reduction = "harmony", dims = 1:opt$dimss, verbose = F)Quality Control Visualizationr# Generate QC plots to assess batch integration quality
...
qsave(sce, file = paste0(opt$cellTY, "_", opt$dimss, "_", opt$batchss, 
                         "_before_sce.harmony.qs"))Multi-resolution Clusteringr# Perform clustering at multiple resolutions as mentioned in methodology:
...
qsave(sce, file = paste0(opt$cellTY, "_", opt$dimss, "_", opt$batchss, 
                         "_batch_cov_sce_qc_after_findcluster.qs"))Clustering Resolution Comparisonr# Generate comprehensive visualization of all clustering resolutions
